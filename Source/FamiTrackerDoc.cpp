...
BOOL CFamiTrackerDoc::SaveDocument(LPCTSTR lpszPathName) const
{
	...
	GetTempFileName(TempPath, _T("UTM"), 0, TempFile);
...
bool CFamiTrackerDoc::ReadBlock_Patterns(CDocumentFile *pDocFile)
{
	...
	while (!pDocFile->BlockDone()) {
		...
		for (unsigned i = 0; i < Items; ++i) {
			...
			if (m_iFileVersion < 0x8000) {
				// For older versions, change the maximum number of instruments on load from the default of 64
				if (Note->Instrument == 64)
					Note->Instrument = MAX_INSTRUMENTS;
			}
...
CString CFamiTrackerDoc::GetFileTitle() const 
{
	...
	// Remove extension
	if (FileName.Right(4).CompareNoCase(_T(".utm")) == 0)
